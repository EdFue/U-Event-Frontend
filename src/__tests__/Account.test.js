...
describe('<Account /> Component', () => {
  ...
  it('uploads an image when a file is selected', async () => {
    ...
    await waitFor(() => expect(fetchStub.calledWithMatch(`https://u-event-backend-d86136b87ee9.herokuapp.com/images/uploadImage/`)).to.be.true);
  });
  ...
  it('deletes the image when delete icon is clicked', async () => {
    ...
    await waitFor(() => expect(fetchStub.calledWithMatch(`https://u-event-backend-d86136b87ee9.herokuapp.com/images/deleteImage/`)).to.be.true);
  ...
  it('handles network error on fetching image gracefully', async () => {
    fetchStub.withArgs(`https://u-event-backend-d86136b87ee9.herokuapp.com/images/image/${username}`).rejects(new Error("Network Error"));
...
// it('shows error message on failed image upload', async () => {
  ...
  //   fireEvent.change(fileInput, { target: { files: [testFile] } });
  //   fetchStub.withArgs(`https://u-event-backend-d86136b87ee9.herokuapp.com/images/uploadImage/${username}`).rejects(new Error("Upload Failed"));
  ...
  //   fetchStub.withArgs(`https://u-event-backend-d86136b87ee9.herokuapp.com/api/users/${username}`).rejects(new Error("Failed to fetch user data"));
  ...
  //   fireEvent.change(getByLabelText('First Name'), { target: { value: 'John' } });
  //   fetchStub.withArgs(`https://u-event-backend-d86136b87ee9.herokuapp.com/api/users/${localStorage.getItem("username")}`).rejects(new Error("Update Failed"));
